...
export default function LoginScreen() {
  ...
  const canSubmit = mobileDigits.length === 10 && password.length >= 6 && !loading;
  ...
  async function handleLogin() {
    ...
    try {
      ...
      await AsyncStorage.setItem('ims_bound_mobile', mobileDigits);
    } catch (err: any) {
      setLoginError(err?.response?.data?.message || 'Unable to login. Please contact admin.');
    } finally {
      setLoading(false);
    }
  }
